...
import { messageService, conversationService, createNewConversation } from "@/lib/database/messages"
...
function AgentMessagesContent() {
  ...
  const filteredConversations = conversations.filter(conversation => {
    if (!searchTerm) {
      ...
      if (propertyId) {
        return conversation.propertyId === propertyId || !conversation.propertyId
      }
      return true
    }
  ...
  const sortedConversations = propertyId ? 
    filteredConversations.sort((a, b) => {
      if (a.propertyId === propertyId && b.propertyId !== propertyId) return -1
      if (b.propertyId === propertyId && a.propertyId !== propertyId) return 1
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await messageService.sendMessage(
        selectedConversation.id,
        userData.uid,
        `${userData.firstName} ${userData.lastName}` || userData.email || 'Agent',
        'agent',
        {
  ...
  const handleUserSearch = async (term: string) => {
    ...
    setIsSearching(true)
    try {
      const results = await searchUsers(term, ['tenant', 'landlord'], 10)
  ...
  const getOtherParticipant = (conversation: Conversation): Participant | null => {
    return conversation.participants.find(p => p.id !== userData?.uid) || null
  }
  ...
  const handleFileUpload = async (file: File) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      alert('Please select a valid file type (images, PDF, DOCX, or Excel files)')
      return
    }
              ...
              {isLoading ? (
                ...
                <div className="text-center py-8 px-4">
                  ...
                  <p className="text-gray-600 dark:text-gray-400 text-sm">
                    {searchTerm ? 'No conversations match your search.' : 'Your conversations with clients will appear here.'}
                  </p>
                </div>
              ) : (
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {getOtherParticipant(selectedConversation)?.type || 'User'}
                      </p>
                    </div>
                  </div>
      ...
      <Dialog open={!!selectedImage} onOpenChange={() => setSelectedImage(null)}>
        <DialogContent className="max-w-[90vw] max-h-[90vh] p-0 overflow-hidden">
          <DialogHeader className="p-4 pb-0 flex-shrink-0">
            <DialogTitle className="truncate pr-8 text-sm">{selectedImage?.name}</DialogTitle>
          </DialogHeader>
