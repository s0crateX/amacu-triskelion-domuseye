...
function LandlordMessagesContent() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await messageService.sendMessage(
        selectedConversation.id,
        userData.uid,
        `${userData.firstName} ${userData.lastName}` || userData.email || 'Landlord',
        'landlord',
        {
  ...
  const handleUserSearch = async (searchTerm: string) => {
    ...
    setIsSearching(true)
    try {
      // Landlords can message agents and tenants
      const results = await searchUsers(searchTerm, ['agent', 'tenant'])
  ...
  const handleCreateConversation = async (otherUser: SearchUser) => {
    ...
    try {
      const conversationId = await conversationService.createNewConversation(
        ...
        otherUser.userType as 'agent' | 'tenant',
  ...
  const getOtherParticipant = (conversation: Conversation | null): Participant | undefined => {
  ...
  const handleDeleteConversation = async (conversationId: string) => {
    ...
    try {
      await conversationService.deleteConversation(conversationId, userData.uid, 'landlord')
    ...
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-2">
          Communicate with your agents and tenants
        </p>
      </div>
              ...
              <Dialog open={showNewConversation} onOpenChange={setShowNewConversation}>
                ...
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Search for agents or tenants to start a conversation
                    </DialogDescription>
                  </DialogHeader>
                  ...
                  {filteredConversations.map((conversation) => {
                    ...
                    return (
                      <div
                        key={conversation.id}
                        className={cn(
                          "flex items-center space-x-3 p-3 hover:bg-muted/50 cursor-pointer border-l-2 transition-colors",
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {conversation.propertyTitle && (
                            <p className="text-xs text-muted-foreground truncate">
                              Property: {truncateText(conversation.propertyTitle, 20)}
                            </p>
                          )}
                          {conversation.lastMessage && (
                            <p className={cn(
                              ...
                              {truncateText(conversation.lastMessage?.content, 20)}
                            </p>
                          )}
                        ...
                        >
                          ...
                          >
                            <Avatar className="w-8 h-8 flex-shrink-0">
                              ...
                              <AvatarFallback>
                                {isOwnMessage 
                                  ? userData?.firstName?.charAt(0)?.toUpperCase() || 'L'
                                  : getOtherParticipant(selectedConversation)?.name?.charAt(0)?.toUpperCase() || 'U'
                                }
                              </AvatarFallback>
                            </Avatar>
