...
export default function AddPropertyForm({ 
  ...
  const handleLocationSave = async (locationData: {
    ...
    const addressParts = locationData.location_address.split(',').map(part => part.trim());
    const extractedLocation = addressParts.length >= 2 ? addressParts[addressParts.length - 2] : addressParts[0] || "General Santos City";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user || !userData) {
      toast.error(`You must be logged in to ${isEditMode ? 'update' : 'add'} a property`);
      return;
    }
    ...
    try {
      ...
      if (!propertyLocation && formData.address) {
        ...
        propertyLocation = addressParts.length >= 2 ? addressParts[addressParts.length - 2] : addressParts[0] || "General Santos City";
      ...
      const propertyData: Omit<Property, 'id'> = {
        ...
        datePosted: isEditMode ? propertyToEdit?.datePosted ?? new Date().toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        ...
        status: isEditMode ? propertyToEdit?.status ?? 'Available' : 'Available',
                ...
                <TabsContent value="basic" className="space-y-8 mt-6">
                  ...
                  <div className="space-y-6">
                    <div className="border-l-4 border-primary pl-4">
                      <h3 className="text-lg font-semibold mb-1">Property Overview</h3>
                      <p className="text-sm text-muted-foreground">Basic information about your property</p>
                    </div>
                  ...
                  <div className="space-y-6">
                    <div className="border-l-4 border-green-500 pl-4">
                      <h3 className="text-lg font-semibold mb-1">Pricing & Location</h3>
                      <p className="text-sm text-muted-foreground">Set your rental price and property location</p>
                    </div>
                      ...
                      {formData.latitude !== 0 && formData.longitude !== 0 && (
                        <div className="flex items-center gap-2 mt-2 p-2 bg-green-50 dark:bg-green-950/20 rounded-md">
                          ...
                          <p className="text-xs text-green-700 dark:text-green-400">
                            Location pinned: {formData.latitude.toFixed(6)}, {formData.longitude.toFixed(6)}
                          </p>
                        </div>
                      )}
                    </div>
                ...
                <TabsContent value="details" className="space-y-8 mt-6">
                  ...
                  <div className="space-y-6">
                    <div className="border-l-4 border-blue-500 pl-4">
                      <h3 className="text-lg font-semibold mb-1">Property Specifications</h3>
                      <p className="text-sm text-muted-foreground">Detailed information about rooms and space</p>
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                      <div className="space-y-2">
                        <Label htmlFor="beds" className="text-xs font-medium flex items-center gap-1">
                          Bedrooms
                          <span className="text-red-500">*</span>
                        </Label>
                        <Input
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="baths" className="text-xs font-medium flex items-center gap-1">
                          Bathrooms
                          <span className="text-red-500">*</span>
                        </Label>
                        <Input
                      ...
                      <div className="space-y-2">
                        ...
                        <Select value={formData.kitchen} onValueChange={(value) => handleInputChange("kitchen", value)}>
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="No Kitchen">No Kitchen</SelectItem>
                            <SelectItem value="Shared Kitchen">Shared Kitchen</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                    </div>
                  ...
                  <div className="space-y-6">
                    <div className="border-l-4 border-purple-500 pl-4">
                      <h3 className="text-lg font-semibold mb-1">Features & Amenities</h3>
                      <p className="text-sm text-muted-foreground">Select all amenities available in your property</p>
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      {amenitiesList.map((amenity) => (
                        <div key={amenity.id} className="flex items-center space-x-2 p-2.5 border rounded-lg hover:bg-muted/50 transition-colors">
                          <Checkbox
                            id={amenity.id}
                            checked={formData.features.includes(amenity.id)}
                            onCheckedChange={(checked) => handleFeatureChange(amenity.id, checked as boolean)}
                            className="flex-shrink-0"
                          />
                          <Label htmlFor={amenity.id} className="flex items-center gap-2 cursor-pointer flex-1 min-w-0">
                ...
                <TabsContent value="images" className="space-y-8 mt-6">
                  <div className="space-y-6">
                    <div className="border-l-4 border-orange-500 pl-4">
                      <h3 className="text-lg font-semibold mb-1">Property Photos</h3>
                      <p className="text-sm text-muted-foreground">Upload high-quality images to showcase your property</p>
                    </div>
                    ...
                    <div className="space-y-6">
                      <div className="border-2 border-dashed border-muted-foreground/25 rounded-xl p-8 text-center hover:border-muted-foreground/40 transition-colors">
                        ...
                        <div className="space-y-3">
                          <div>
                            <h4 className="text-lg font-medium mb-1">Upload Property Images</h4>
                            <p className="text-sm text-muted-foreground">
                              Add photos of different rooms, exterior views, and key features
                            </p>
                          </div>
                          <IKUpload
